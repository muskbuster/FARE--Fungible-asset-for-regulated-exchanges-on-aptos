...
module FARE::compliance_and_dvp_tests {
    ...
    fun create_test_identity(user: &signer) {
        let user_addr = signer::address_of(user);
        // Create account for user first
        account::create_account_for_test(user_addr);
        // Only create identity if it doesn't already exist
        if (!onchain_identity::has_identity(user_addr)) {
    ...
    fun mint_tokens_to_user(admin: &signer, user: address, token_address: address, amount: u64) {
        // Mint tokens using the new mint function
        trex_token::mint_tokens(admin, token_address, user, amount);
    }
    ...
    public fun test_compliant_transfer(admin: &signer, user1: &signer, user2: &signer) {
        ...
        create_test_identity(user1);
        create_test_identity(user2);
